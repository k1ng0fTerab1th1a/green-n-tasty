    ...
    : IClassFixture<DynamoDbFixture>
{
    ...
    [Fact]
    public async Task IsFeedbackAlreadyMade_WhenFeedbackExists_ShouldReturnTrue()
    {
        var reservationId = Guid.NewGuid().ToString("N");
        var locationId    = $"loc-{Guid.NewGuid():N}";
        ...
        var result = await _repo.IsFeedbackAlreadyMade(reservationId, "waiter", CancellationToken.None);

        result.Should().BeTrue();
    }

    [Fact]
    public async Task IsFeedbackAlreadyMade_WhenNoFeedbackExists_ShouldReturnFalse()
    {
        var reservationId = Guid.NewGuid().ToString("N"); // never saved
    ...
    [Fact]
    public async Task IsFeedbackAlreadyMade_WhenOnlyOtherTypeExists_ShouldReturnFalse()
    {
        var reservationId = Guid.NewGuid().ToString("N");
        var locationId    = $"loc-{Guid.NewGuid():N}";
        ...
        var feedback = new Feedback
        {
            ...
            ReservationId = reservationId,
            Rate          = 4,
            Comment       = "Nice kitchen",
            UserId        = "user-4",
            UserName      = "Giorgi B.",
            UserAvatarUrl = string.Empty,
            Date          = DateTimeOffset.UtcNow.ToString("o"),
            LocationId    = locationId,
            Type          = "kitchen"   // kitchen saved, but we'll check for waiter
        };
    ...
    [Fact]
    public async Task IsFeedbackAlreadyMade_WhenBothTypesExist_ShouldReturnTrueForEachType()
    {
        var reservationId = Guid.NewGuid().ToString("N");
        var locationId    = $"loc-{Guid.NewGuid():N}";
        ...
        var waiterResult  = await _repo.IsFeedbackAlreadyMade(reservationId, "waiter",  CancellationToken.None);
        var kitchenResult = await _repo.IsFeedbackAlreadyMade(reservationId, "kitchen", CancellationToken.None);

        waiterResult.Should().BeTrue();
        kitchenResult.Should().BeTrue();
    }
